...
// Gửi email dùng chung cho MỌI luồng (liên hệ/báo giá/báo lỗi thi công).
// Trả { status, body } y như handleSubmit để route chỉ việc chuyển tiếp.
async function deliver({ subject, text, html, attachments, replyTo }) {
  ...
  try {
    ...
    // Log full detail server-side (visible in the log stream) but never leak it to clients.
    console.error('Failed to send email:', err && (err.response || err.message), err && err.code);
    // Gmail hỏng thì yêu cầu của khách KHÔNG được biến mất — ghi ra đĩa để còn
    // gọi lại cho khách. Health check hằng ngày sẽ báo động nếu thư mục có tệp.
    spoolFailedMail({ subject, text, replyTo, attachments, err });
    return {
      status: 502,
      body: {
        ok: false,
...
// Lưu nội dung email không gửi được vào Private/failed-mail/<thời-điểm>-<slug>/.
// Bọc try/catch riêng: lỗi khi lưu tuyệt đối không được che lỗi gửi mail gốc.
function spoolFailedMail({ subject, text, replyTo, attachments, err }) {
  try {
    const at = new Date().toISOString().replace(/[:.]/g, '-');
    const slug = String(subject || 'mail')
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 60) || 'mail';
    const dir = path.join(__dirname, '..', 'Private', 'failed-mail', `${at}-${slug}`);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'mail.json'), JSON.stringify({
      at: new Date().toISOString(),
      subject, text, replyTo: replyTo || null,
      error: (err && (err.response || err.message)) || String(err),
      attachments: (attachments || []).map((a) => a.filename),
    }, null, 2) + '\n');
    (attachments || []).forEach((a, i) => {
      if (!a || !a.content) return;
      const safe = String(a.filename || `tep-${i + 1}`).replace(/[/\\]/g, '_');
      fs.writeFileSync(path.join(dir, safe), a.content);
    });
    console.error('Đã lưu email lỗi vào', dir);
  } catch (spoolErr) {
    console.error('Không lưu được email lỗi:', spoolErr && spoolErr.message);
  }
}
...
async function handleSubmit(data, files) {
  ...
  return deliver({
    subject: `[Sunwa Design] Liên hệ Tư vấn — ${name} (${phone})`,
    text: lines.join('\n'),
    html,
    attachments,
...
module.exports = { handleSubmit, deliver, clean, esc };
