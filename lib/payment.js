...
async function handleCreateOrder(req, res) {
  if (!configured()) {
    return res.status(500).json({ ok: false, error: 'Thanh toán chưa được cấu hình. Vui lòng dùng form Liên hệ Tư vấn hoặc gọi hotline 0916 557 558.' });
  }
  const body = req.body || {};
...
async function processWebhook(req, res) {
  ...
  if (tx.transferType && tx.transferType !== 'in') return res.json({ success: true, ok: true, skip: 'not-in' });
  ...
  if (!order) {
    ...
    return res.json({ success: true, ok: true, skip: 'no-match' });
  }
  if (order.status === 'paid') return res.json({ success: true, ok: true, skip: 'already-paid' });
  if (Number(tx.transferAmount) < order.fee) {
    fs.appendFileSync(path.join(ORDERS_DIR, 'unmatched.log'),
      JSON.stringify({ at: new Date().toISOString(), reason: 'thieu-tien', code: order.code, tx }) + '\n');
    return res.json({ success: true, ok: true, skip: 'amount-too-low' });
  }
