...
// Mặc định tính bằng THÁNG: kết cấu 10 năm · chống thấm 3 năm · hoàn thiện 1 năm.
const WARRANTY_DEFAULT_MONTHS = { ketCau: 120, chongTham: 36, hoanThien: 12 };
...
function handleLogin(req, res) {
  ...
  if (!PHONE_REGEX.test(phone)) return fail();

  // Đăng nhập chỉ bằng SĐT → tìm khách theo số (mỗi khách một số duy nhất).
  // So sánh mọi mục (không dừng sớm) để thời gian phản hồi không lộ thông tin.
  const clients = loadClients();
  let found = null;
  for (const [code, client] of Object.entries(clients)) {
    if (timingEqual(phone, String(client.phone || '')) && !found) found = { code, client };
  }
